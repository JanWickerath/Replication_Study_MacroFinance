...
y3 = simult_(initial_condition_states,oo_.dr,horzcat(shck.tfp_resid, zeros(length(shck.fin_resid),1)),1);

y4 = simult_(initial_condition_states,oo_.dr,horzcat(zeros(length(shck.tfp_resid),1), shck.fin_resid),1);
